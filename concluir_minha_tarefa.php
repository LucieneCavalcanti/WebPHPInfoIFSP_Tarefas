        ...
        if(isset($_GET['id'])){
            ...
            if($checkTarefa->rowCount() > 0) {
                ...
                if(isset($_POST['confirmar']) && $_POST['confirmar'] == '1') {
                    ...
                    $stmt = $conn->prepare("UPDATE tarefa_usuario SET status = 'Concluída' WHERE id = :id");
                        ...
                        <div class="col-md-8 offset-md-2">
                            <div class="card">
                                <div class="card-header bg-primary text-white">
                                    <h4 class="mb-0">Concluir Tarefa</h4>
                                </div>
                                <div class="card-body">
                                    <div class="alert alert-info">
                                        <p>Você está prestes a marcar a seguinte tarefa como concluída:</p>
                                        <p><strong>ID:</strong> <?php echo $tarefa['tarefa_id']; ?></p>
                                        <p><strong>Descrição:</strong> <?php echo htmlspecialchars($tarefa['descricao']); ?></p>
                                    </div>
                                    
                                    <form action="concluir_minha_tarefa.php?id=<?php echo $id; ?>" method="post">
                                        <input type="hidden" name="confirmar" value="1">
                                        
                                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                            <button type="submit" class="btn btn-success me-md-2">Confirmar Conclusão</button>
                ...
                echo "<div class='alert alert-warning'>Tarefa não encontrada, já concluída/cancelada ou você não tem permissão para concluí-la. <a href='minhas_tarefas.php' class='alert-link'>Voltar para minhas tarefas</a></div>";
            }
        } else {
