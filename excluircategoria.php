...
if(isset($_SESSION['tipoUsuario']) && $_SESSION['tipoUsuario']=="Administrador"){
    ...
    try{
        if(isset($_GET['id'])){
            ...
            $checkStmt = $conn->prepare("SELECT id FROM categoria WHERE id = :id");
            $checkStmt->bindParam(":id", $id, PDO::PARAM_INT);
            $checkStmt->execute();
            
            if($checkStmt->rowCount() > 0) {
                // Categoria existe, pode excluir
                // Devido à restrição de chave estrangeira com CASCADE, as tarefas relacionadas serão excluídas automaticamente
                ...
                if($linhas_deletadas == 1) {
                    ...
                    <div class="alert alert-warning alert-dismissible fade show" role="alert">
                        <strong>Atenção!</strong> Erro ao excluir a categoria.
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>
                    <div class="text-center mt-3">
                        <a href="listacategorias.php" class="btn btn-primary">Voltar para a listagem</a>
                ...
                ?>
                <div class="alert alert-warning alert-dismissible fade show" role="alert">
                    <strong>Atenção!</strong> Categoria não encontrada.
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                </div>
                <div class="text-center mt-3">
                    <a href="listacategorias.php" class="btn btn-primary">Voltar para a listagem</a>
            ...
            ?>
            <div class="alert alert-warning alert-dismissible fade show" role="alert">
                <strong>Atenção!</strong> Você precisa selecionar uma categoria.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
            <div class="text-center mt-3">
                <a href="listacategorias.php" class="btn btn-primary">Voltar para a listagem</a>
